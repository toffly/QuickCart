...
const autoAssignRider = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    // Wait 5 minute before attempting assignment
    ...
    const result = await step.run("assign-rider", async () => {
      ...
      const budyOrders = await prisma.order.findMany({
        where: {
      ...
      const busyRiderIds = budyOrders.map((o) => o.deliveryPartnerId);
